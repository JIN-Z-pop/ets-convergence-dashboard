...
入り、stub充足済み後の毎回実行で誤abortする時制バグを発見(葉山「修正せず報告」判断→奏物的確認
→追加裁定)。Guard 1を2状態許容の冪等ガードへ改訂:
  状態A: NULL行=0件 → stub充足済み=正常。Step A(UPDATE)をskipしStep B(差分INSERT)のみ実行。
  状態B: NULL行=正確に[2026-04-22]の1件 → 従来どおりStep A実行後Step B。
  それ以外(想定外dateのNULL等) → ABORT維持(裁定持ち帰り、現行文言のまま)。

安全ガード:
  1. 実行前に price_usd IS NULL 行を全走査 — 状態A/Bのいずれでもなければ即abort(裁定持ち帰り)。
  2. UPDATE文は WHERE date=? AND price_usd IS NULL の複合条件(誤爆構造的に不能、状態Bのみ実行)。
  3. date<2026-04-22 の全行についてsync前後でhash比較し不変性を検証(状態A/B共通・不可侵)。
...
def main():
    ...
    # --- Guard 1 (S1a-2, 冪等): pre-scan NULL price_usd rows — allow 2 states ---
    null_rows = gods.execute(
        "SELECT date, price_usd, volume, fetched_at FROM raw_eu_ets_daily WHERE price_usd IS NULL"
    ).fetchall()
    if len(null_rows) == 0:
        state = "A"
        print("[guard1] NULL price_usd pre-scan: OK (0 rows, state=A: stub既充足)")
    elif len(null_rows) == 1 and null_rows[0][0] == STUB_DATE:
        state = "B"
    ...
    if state == "B":
        ...
        print("[stepA] skipped (state=A: stub既充足のためStep Bのみ実行)")
    ...
    # 2026-08-01鬼検証: close_priceだけを見ていたが、下流で int(v) も撃つためvolume単独NULLでも
    # 同型に落ちる。float()/int()を掛ける列を過不足なく判定列にする(検査対象=書込み対象)。
    incomplete_rows = [(d, c, v) for d, c, v in src_rows if c is None or v is None]
    if incomplete_rows:
        print(f"[skip] {len(incomplete_rows)} row(s) with NULL close_price/volume in eu_ets_daily — not "
              f"genuine observations (source gap), excluded from sync (not inserted as-is): {incomplete_rows}")
